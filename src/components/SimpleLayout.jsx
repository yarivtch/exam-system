...
const SimpleLayout = ({
  ...
  return (
    <div className="h-screen bg-gray-50 flex">
      ...
      {sidebarOpen && (
        <div className="w-1/4 bg-white border-r border-gray-200 flex flex-col">
          ...
          <div className="p-4 border-t border-gray-100">
            ...
            >
              {answeredQuestions === totalQuestions ? '✅ סיים מבחן' : '⚠️ סיים מבחן'}
              <div className="text-xs mt-1">
                {answeredQuestions === totalQuestions 
                  ? 'כל השאלות נענו'
                  : `נותרו ${totalQuestions - answeredQuestions} שאלות`
                }
              </div>
            </button>
          </div>
      ...
      {/* Main Content - 75% או 100% */}
      <div className={`${sidebarOpen ? 'w-3/4' : 'w-full'} flex flex-col`}>
          ...
          {category.questions.map((question) => {
            ...
            return (
              <button
                ...
                className={`
                  w-full text-right p-2 rounded-md transition-colors text-sm
                  flex items-center justify-between
                  ${isCurrent 
                    ? 'bg-blue-100 text-blue-900 font-medium border-2 border-blue-300' 
                    : isAnswered
                    ? 'bg-green-50 text-green-800 hover:bg-green-100'
                    : 'text-gray-600 hover:bg-gray-100 bg-red-50'
                  }
                `}
              >
                ...
                <div className="flex items-center space-x-1 space-x-reverse">
                  ...
                  {isCurrent && (
                    <span className="text-xs bg-blue-500 text-white px-1 rounded">נוכחי</span>
                  )}
                </div>
              </button>
            );
